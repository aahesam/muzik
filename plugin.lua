...
ارسال پست با SMS:
`یک قابلیت ویژه میباشد و از طریق آن میتوانید با پیامک به کانالتان پست های متنی ارسال کنید. دقت کنید که قابلیت مارک داون فعال است و میتوانید از الگوهای مارک داون استفاده نمایید. حد اکثر متن باید 4000 کاراکتر باشد. متن خود را به صورت پیام کوتاه یا همان اس ام اس به شماره ای که هنگام ثبت نام ارائه نمودید ارسال کنید تا در کانالتان پست شود. پست ممکن است تا یک دقیقه با تاخیر انجام شود.`
...
`با استفاده از این کلید میتوانید نام کانال، یوزرنیم کانال، آی دی کانال، شماره ارتباطی با سامانه پیامکی، انقضای اکانت، آمار کانال، سازنده ی کانال، تعداد ادمینهای کانال، لیست ادمینهای کانال را در یک لیست ببینید.`]]
	...
	elseif msg.text == "مثال کیبرد اینلاین و مارک داون" then
		inlinekey = {{{text = "کانال تیم" , url = "https://telegram.me/khabar_arz"}},{{text = "ربات تیم" , url = "https://telegram.me/Modiriat_channalbot"}}}
		text = "مثال استایل نویس با فونتهای مختلف:\n*MDesign Team*\nMDesign Team\n_MDesign Team_\n`MDesign Team`\nتیم ربات\n`تیم رباتساز`\nمثال هایپر لینک:\n[تیم رباتساز](telegram.me/khabar_arz)\n[MDesign Team](telegram.me/khabar_arz)\nمثال کیبرد اینلاین:"
	...
	if msg.text:find('/add') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/add 12345678,@khabar_arz,9131234567*\n`/add [telegram id],[@channel],[number]`", true)
		end
	elseif msg.text:find('/edit') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/edit 12345678,@khabar_arz,9131234567*\n`/edit [telegram id],[@channel],[number]`", true)
		end
	...
	elseif msg.text == "مثال کیبرد اینلاین" then
		return send_inline(msg.from.id, "`تیتر کیبرد اینلاین با قابلیت استفاده از قابلیت مارک داون`", {{{text = "کانال تیم" , url = "https://telegram.me/khabar_arz"}}})
	end
